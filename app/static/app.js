...
async function load_networks(){const a=await api('/api/networks');$('#content').innerHTML=`<div class=toolbar><button onclick="networkDialog()">+ Создать сеть</button><button class=secondary onclick="prune('networks')">Очистить неиспользуемые</button></div><table><thead><tr><th>Имя</th><th>Драйвер</th><th>Параметры</th><th>Контейнеры</th><th></th></tr></thead><tbody>${a.map(x=>`<tr><td><b>${esc(x.name)}</b><br><span class=muted>${esc(x.id)}</span></td><td>${esc(x.driver||'')}</td><td>${x.internal?'internal ':''}${x.attachable?'attachable':''}<br><span class=muted>${(x.subnets||[]).map(s=>esc(s.Subnet||'')).join(', ')}</span></td><td>${(x.containers||[]).length}</td><td class=actions><button class=secondary onclick="editNetwork('${x.id}')">Редактировать</button><button class=danger onclick="removeNetwork('${x.id}')">Удалить</button></td></tr>`).join('')}</tbody></table>`}
async function editNetwork(id){
 try{
  const [networks,containers]=await Promise.all([api('/api/networks'),api('/api/containers')]),n=networks.find(x=>x.id===id);
  if(!n)throw Error('Сеть не найдена');
  const attached=n.containers||[],attachedNames=new Set(attached.map(x=>x.Name)),available=containers.filter(x=>!attachedNames.has(x.name)),subnet=(n.subnets||[])[0]||{},system=['bridge','host','none'].includes(n.name);
  modal(`<div class=network-card><h2>Сеть: ${esc(n.name)}</h2><div class=network-meta><span>ID: <code>${esc(n.id)}</code></span><span>Scope: <b>${esc(n.scope||'local')}</b></span></div>
  ${system?'<div class=warning>Это системная сеть Docker. Её конфигурацию нельзя пересоздать из панели.</div>':''}
  <h3>Конфигурация</h3><form class=form-grid onsubmit="saveNetwork(event,'${n.id}')">
  <label>Имя<input id=e_name value="${esc(n.name)}" required ${system?'disabled':''}></label>
  <label>Драйвер<select id=e_driver ${system?'disabled':''}><option value=bridge ${n.driver==='bridge'?'selected':''}>bridge</option></select></label>
  <label>Subnet<input id=e_subnet value="${esc(subnet.Subnet||'')}" placeholder="172.30.0.0/16" ${system?'disabled':''}></label>
  <label>Gateway<input id=e_gateway value="${esc(subnet.Gateway||'')}" placeholder="172.30.0.1" ${system?'disabled':''}></label>
  <label><input id=e_internal type=checkbox style="width:auto" ${n.internal?'checked':''} ${system?'disabled':''}> Internal</label>
  <label><input id=e_attachable type=checkbox style="width:auto" ${n.attachable?'checked':''} ${system?'disabled':''}> Attachable</label>
  ${!system?`<button class="full danger" ${attached.length?'disabled title="Сначала отключите контейнеры"':''}>Пересоздать сеть с конфигурацией</button>`:''}</form>
  <h3>Подключённые контейнеры</h3><div class=network-connections>${attached.map(x=>`<div><span><b>${esc(x.Name||'')}</b><br><small>${esc(x.IPv4Address||x.IPv6Address||'без IP')}</small></span><button class=danger onclick="disconnectNetwork('${n.id}','${encodeURIComponent(x.Name||'')}')">Отключить</button></div>`).join('')||'<p class=muted>Нет подключённых контейнеров</p>'}</div>
  <h3>Подключить контейнер</h3><div class=inline-form><select id=e_container>${available.map(x=>`<option value="${esc(x.id)}">${esc(x.name)}</option>`).join('')}</select><button onclick="connectNetwork('${n.id}')" ${available.length?'':'disabled'}>Подключить</button></div></div>`);
 }catch(e){toast(e.message)}
}
async function saveNetwork(e,id){e.preventDefault();if(!confirm('Docker пересоздаст сеть. Продолжить?'))return;try{await api(`/api/networks/${id}`,{method:'PUT',body:JSON.stringify({name:e_name.value,driver:e_driver.value,subnet:e_subnet.value||null,gateway:e_gateway.value||null,internal:e_internal.checked,attachable:e_attachable.checked})});closeModal();load_networks();toast('Сеть обновлена')}catch(e){toast(e.message)}}
async function connectNetwork(id){if(!e_container.value)return;try{await api(`/api/networks/${id}/connect`,{method:'POST',body:JSON.stringify({container:e_container.value})});editNetwork(id)}catch(e){toast(e.message)}}
async function disconnectNetwork(id,container){if(!confirm('Отключить контейнер от сети?'))return;try{await api(`/api/networks/${id}/disconnect?force=true`,{method:'POST',body:JSON.stringify({container:decodeURIComponent(container)})});editNetwork(id)}catch(e){toast(e.message)}}
