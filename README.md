# DockPilot v1.2.0 — веб-панель управления Docker
...
- JWT-авторизация;
- просмотр фактических runtime-переменных окружения контейнера с маскировкой значений;
- графики CPU, суммарного сетевого трафика контейнеров и занятости диска на странице обзора;
- адаптивный тёмный web-интерфейс.
- ручные снимки образов запущенных контейнеров и резервное копирование по расписанию;
- хранение копий в подключённом Docker-томе или интерактивно настроенном WebDAV.

## Резервное копирование

Раздел **Резервные копии** создаёт снимок writable layer выбранного запущенного
контейнера, сохраняет его как переносимый архив Docker image (`.tar`) и добавляет
JSON-манифест с конфигурацией контейнера. Архив можно восстановить командой:

```bash
docker load -i /path/to/container-YYYYMMDD-HHMMSS.tar
```

По умолчанию локальные копии находятся в томе `dockpilot_backups`, подключённом
к `/backups`. Для сетевого хранилища переопределите этот том в
`docker-compose.override.yml`, например для NFS:
...
volumes:
  dockpilot_backups:
    ...
    driver_opts:
      ...
      device: ":/exports/dockpilot"
```

WebDAV можно подключить прямо в интерфейсе: укажите URL, логин, пароль и удалённый
каталог, нажмите **Проверить WebDAV**, сохраните настройки и включите расписание.
Пароль сохраняется с правами `0600` в постоянном томе данных и никогда не
возвращается API или браузеру. Для WebDAV с собственным центром сертификации
добавьте его CA-сертификат в доверенные сертификаты контейнера; отключение
проверки TLS намеренно не поддерживается.

Снимок образа не заменяет согласованную резервную копию данных в Docker volumes.
Для PostgreSQL, MariaDB и других СУБД дополнительно используйте штатные средства
дампа либо остановите запись приложений на время копирования данных томов.
...
Просмотр runtime `.env` может раскрыть пароли и токены контейнеров. Значения скрыты
по умолчанию, но доступны авторизованному администратору панели. Ограничивайте доступ
к DockPilot так же строго, как доступ к root и Docker socket.
